...
int32_t *Encoder::getDeprecatedDataLeft() {
    int ret = av_frame_make_writable(frame);
    if (ret < 0) {
        exit(1);
    }
    return (int32_t *) frame->data[0];
}

int32_t *Encoder::getDeprecatedDataRight() {
    int ret = av_frame_make_writable(frame);
    if (ret < 0) {
        exit(1);
    }
    return (int32_t *) frame->data[1];
}

int Encoder::channels() {
    return c->channels;
}
